    ...
    <div id="how-we-work" className="pt-16 px-8">
      ...
      <div className="max-w-6xl mx-auto" ref={sectionRef}>
        <div className="relative">
          <div className={`hidden lg:block absolute top-6 md:top-8 left-0 right-0 h-1 rounded-[5px] ${isVisible && "line-animate"}`} />
          
          <div className={`block lg:hidden sm:hidden absolute top-0 bottom-0 left-6 w-[3px] rounded-[5px] ${isVisible && "line-animate"}`} style={{height: "auto"}} />
          
          <div className="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 md:gap-8">
            {steps.map((step, index) => (
              <div key={step} className="flex sm:flex-col items-center sm:items-center gap-3 sm:gap-0">
      ...
      <div className="flex mt-8 sm:mt-12 justify-start sm:justify-center">
        <button className="flex items-center gap-2 text-sm md:text-xl btn-hover rounded-[5px] px-8 py-3 font-semibold bg-primary text-white transition">
          Поискай оферта
        </button>
      </div>
    </div>
